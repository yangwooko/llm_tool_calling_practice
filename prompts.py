...
# 법령 충분성 검사 프롬프트
LAW_SUFFICIENCY_PROMPT = """당신은 법령 내용이 사용자의 질문에 답변하기에 충분한지 판단하는 전문가입니다.
...
# 추가 검색 필요성 판단 프롬프트
ADDITIONAL_SEARCH_PROMPT = """당신은 법령 내용을 분석하여 사용자의 질문에 답하기 위해 추가 검색이 필요한지 판단하는 전문가입니다.
...
관련성 판단 예시:
사용자 질문: "건축법에서 경미한 사항의 변경이 구체적으로 무엇인지 알려줘"

✅ 관련 있는 법령:
"대통령령으로 정하는 경미한 사항의 변경은 그러하지 아니하다."
→ "경미한 사항의 변경" 키워드가 직접 포함됨

❌ 관련 없는 법령:
"건축물의 유지관리에 관한 세부사항은 대통령령으로 정한다."
→ "경미한 사항의 변경"과 관련 없음

❌ 관련 없는 법령:
"건축물의 높이 제한은 대통령령으로 정한다."
→ "경미한 사항의 변경"과 관련 없음

❌ 관련 없는 법령:
"건축물의 용도 변경에 관한 세부사항은 시행령으로 정한다."
→ "경미한 사항의 변경"과 관련 없음

사용자 질문: "건축물의 높이 제한에 대해 알려줘"
...
❌ 관련 없는 법령:
"경미한 사항의 변경은 대통령령으로 정한다."
→ "높이 제한"과 관련 없음

사용자 질문: "건축물의 용도 변경 절차에 대해 알려줘"
...
답변 형식:
- "추가 검색 필요": 사용자 질문과 직접 관련이 있고, 추가 검색이 필요한 경우
- "추가 검색 불필요": 사용자 질문과 관련이 없거나, 현재 법령 텍스트로 사용자 질문에 충분히 답할 수 있는 경우

추가 검색이 필요한 경우:
- 검색 대상: [검색할 법령/시행령/시행규칙명]
- 검색 키워드: [검색에 사용할 키워드들]
- 검색 이유: [왜 추가 검색이 필요한지, 사용자 질문과의 연관성]
...
PROMPT_MAPPING = {
    ...
    "law_sufficiency": LAW_SUFFICIENCY_PROMPT,
    "system": SYSTEM_PROMPT,
    "law_expert": LAW_EXPERT_PROMPT,
    "additional_search": ADDITIONAL_SEARCH_PROMPT,
}
...
def generate_prompt(prompt_type: str, **kwargs) -> list:
    ...
    elif prompt_type == "law_sufficiency":
        law_content = kwargs.get("law_content", "")
...
위 법령 내용이 사용자 질문에 답변하기에 충분한지 판단해주세요.""",
    ...
    elif prompt_type == "additional_search":
        law_content = kwargs.get("law_content", "")
        user_question = kwargs.get("user_question", "")
        return [
            {
...
위 법령 내용을 분석하여 사용자의 질문에 답하기 위해 추가 검색이 필요한지 판단해주세요.""",
            },
        ]
